...
Table~\ref{tab:accuracy} presents the CoQA accuracy and model size for each quantization method.
...
NF4 outperforms the FP16 baseline by 5.1 F1 points, a counterintuitive result suggesting quantization noise acts as a regularizer~\cite{askari2022injected}. FP4 underperforms both NF4 and FP16 by a significant margin (8.9 F1 points below NF4), consistent with Lloyd-Max theory~\cite{lloyd1982least}. Both quantized models achieve identical size (965 MB). These results are visualized in Figure~\ref{fig:bar_comparison}.
...
Table~\ref{tab:latency} shows the latency measurements across different sequence lengths.
...
Quantized models exhibit higher latency due to dequantization overhead, as 4-bit weights must be dequantized to FP16 for matrix multiplications. Speculative decoding~\cite{leviathan2023speculative} could potentially offset this overhead in production settings.

\subsection{Throughput Results}

Table~\ref{tab:throughput} reports throughput across different batch sizes, with results visualized in Figure~\ref{fig:throughput}.
